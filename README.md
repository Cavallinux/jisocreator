...
**Current Test Statistics**: 278 tests total across 63 test classes, all passing (`mvn -o clean test`: 278/278, 0 skipped; `mvn -o clean test -Pwindows`: 278/278, 7 skipped, 0 failures — see "Platform-mismatch note" below).
...
The test suite (278 tests / 63 classes, see [TESTING.md](TESTING.md)) favors SWT-independent coverage so most tests run headlessly without a display:
