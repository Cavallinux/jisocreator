...
**Current Test Statistics**: 281 tests total across 63 test classes, all passing (`mvn -o clean test`: 281/281, 0 skipped; `mvn -o clean test -Pwindows`: 281/281, 7 skipped, 0 failures — see "Platform-mismatch note" below).
...
The test suite (281 tests / 63 classes, see [TESTING.md](TESTING.md)) favors SWT-independent coverage so most tests run headlessly without a display:
