...
### Analyzed — bugfix in progress on `feature/v0.2.3-windowsrootfiles`
- **Windows: fixed-disk root drives missing from the OS explorer tree**: root-cause analysis found that `ShowOnlyDirectoriesFilter` (the `TreeViewer` filter for the OS explorer) calls `OSExplorer#isDirectory(Path)` for every element, including filesystem roots from `File.listRoots()`, without a root-aware special case — unlike its sibling `ShowOnlyIsoDirectoriesFilter`, which already short-circuits root nodes (`node.isRoot() ? true : isDirectory(node)`). Since `Files.isDirectory(Path)` (NIO.2) returns `false` both when a path genuinely isn't a directory and when any I/O error occurs reading its attributes, a fixed disk blocked by Windows Defender's "Controlled folder access" (or a locked BitLocker volume, restrictive NTFS ACLs, etc.) gets silently filtered out of the tree, while removable media mounted afterward — typically not subject to the same OS-level restriction — shows up correctly. See the new "Known Issues" section in `README.md` for full details and the correction plan (distinguishing `AccessDeniedException` from other I/O errors for root paths, then applying the same root short-circuit already used by `ShowOnlyIsoDirectoriesFilter`). Being implemented and tested on the dedicated `feature/v0.2.3-windowsrootfiles` branch before merging back into `feature/v0.2.3`.
