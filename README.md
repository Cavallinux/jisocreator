...
**Current Test Statistics**: 279 tests total across 63 test classes, all passing (`mvn -o clean test`: 279/279, 0 skipped; `mvn -o clean test -Pwindows`: 279/279, 7 skipped, 0 failures — see "Platform-mismatch note" below).
...
The test suite (279 tests / 63 classes, see [TESTING.md](TESTING.md)) favors SWT-independent coverage so most tests run headlessly without a display:
