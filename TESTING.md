...
- **New `submit()` method** (replacing the previous direct `.start()` call from `OSExplorerSashFormSelectionChangedListener`): shows the busy cursor immediately, then swaps the shared `PENDING_TASK` `Future` reference and cancels the *previous* one via `Future#cancel(false)`. Since the executor is single-threaded, a previous task still sitting in the queue (not yet started) is removed from the queue and **never runs at all** — a further improvement over simply discarding a completed task's stale result, since navigating rapidly through several directories no longer wastes background scan work on any of the intermediate ones. A task that has already started running when superseded is left to finish (`cancel(false)` does not interrupt it), but its result is still discarded via the existing `isStillLatestRequest()` check, unchanged from before.
- **`isStillLatestRequest()`**, **`showBusyCursor()`/`restoreDefaultCursor()`**, and the constructor's `LATEST_REQUESTED_DIRECTORY` bookkeeping are otherwise unchanged in behavior — only the mechanism that schedules `run()` changed (executor submission instead of `Thread#start()`).
- `LoadOSDirectoryContentsThreadTest.java` was renamed to **`LoadOSDirectoryContentsTaskTest.java`** (still **4 tests**, unchanged count): `builderShouldProduceRunnableInstance` now asserts `instanceof Runnable` (previously `instanceof Thread`), and the rest of the pure-logic/no-op tests (`isStillLatestRequest*`, `busyCursorMethodsShouldBeNoOpsWithoutTableViewer`) are otherwise unchanged. `submit()`/`run()`'s actual executor+`Display`-dependent behavior remains **not unit-tested**, consistent with the established convention — invoking it would touch the shared `Display` singleton and `OSAndIsoExplorerManager`, risking test-order-dependent pollution across the suite, and this sandbox's live `Display.getDefault()` (backed by an X server) still cannot safely drive `TableViewer#setInput(...)`/GUI state without a full `MainWindow`.
- Validated per this session's standing rule: `mvn -o clean test` (default `linux` profile) → **279/279, 0 skipped**; `mvn -o clean test -Pwindows` (mismatched profile on this Linux host) → **279/279, 7 skipped, 0 failures, no JVM crash** (unchanged from the previous pass — no behavioral change to any tested logic, only the scheduling mechanism).
...
8. **Pending — Full GUI/SWT classes (low priority, needs new infrastructure):** `gui/dialog/*` (`AboutDialog`, `ADDFileToIsoLayoutDialog`, `JISOCreatorPreferencesDialog`, `ShowIsoLayoutInformationDialog`), `gui/window/MainWindow`, `gui/sashfom/*`, `gui/preference/*`, `gui/listeners/*` (6 classes), `gui/decl/ICompositeCreator`, `gui/theme/DarkThemeSupport`, `gui/theme/DarkJFaceDialogs`. Already flagged in "Future Testing Enhancements" below; requires a headless SWT test harness (e.g. Xvfb) before writing tests — do not use partial mocks that break the real-widget architecture.

## Tests Added in this pass (Windows dark mode: system theme detection + user preference toggle)
Following up on the pre-existing forced dark-mode styling (`gui/theme/DarkThemeSupport`/`DarkJFaceDialogs`, unconditionally applied on `win32`), this pass added: (1) a new `jisocreator.theme.mode` user preference (`AUTO`/`LIGHT`/`DARK`, default `AUTO`) exposed via a `StyledComboFieldEditor` in `GeneralPreferencesPage`; (2) `gui/theme/WindowsSystemThemeDetector`, which reads the real Windows system theme from the registry (`HKCU\...\Personalize\AppsUseLightTheme` via `reg query`/`ProcessBuilder`) when the preference is `AUTO`; and (3) `DarkThemeSupport.isDarkModeActive()`, a private gating method now checked at the top of `enableWindowsDarkMode`/`applyToControlTree`/`applyToMainBars` (replacing the previous unconditional `isWin32()`-only check) that resolves `LIGHT`→false, `DARK`→true, `AUTO`/anything else→`WindowsSystemThemeDetector.isSystemDarkMode()`.
- **`WindowsSystemThemeDetector`** was deliberately split into an I/O method (`isSystemDarkMode()`, invokes `reg.exe`, 2s timeout, defaults to light/`false` on any `IOException`/`InterruptedException`) and a pure, package-private static parser (`parseIsDarkMode(String)`, regex-based over the `reg query` stdout). Only the parser is unit-tested — invoking `isSystemDarkMode()` itself would depend on the real registry of whatever host runs the test, which is unverifiable/non-deterministic in CI, so it follows the same pure-logic/I/O split convention used throughout this project (e.g. Group 1/2 above).
- `WindowsSystemThemeDetectorTest.java` (**new, 6 tests**) — dark (`0x0`)/light (`0x1`) parsing, missing-registry-value error output, `null` input, empty/malformed garbage input, and tab-separated output without the registry key header line. All canned strings, no `reg.exe`/registry access, runs identically on any OS.
- `JISOCreatorThemeOptionsTest.java` (**new, 2 tests**) — mirrors `JISOCreatorISOLevelOptionsTest`: asserts the `AUTO`/`LIGHT`/`DARK` enum ordering/preference values and that every enum constant exposes a non-blank localized label.
- **`DarkThemeSupport.isDarkModeActive()` itself remains untested** for the same reason as the rest of `DarkThemeSupport` — see "Test Gap Analysis, Group 8" and the note below on the `dark-theme-unit-tests` feasibility investigation.
- Verified: `mvn -P windows clean test` → **290/290, 0 failures/errors, 2 skipped** (skip count differs from the historical Linux-host baseline above because this pass was run and validated directly on a real Windows host with the `windows` Maven profile, not the Linux/mismatched-profile skip path).

### Open item — headless SWT/`Display` test harness still pending (blocks `DarkThemeSupport`/`DarkJFaceDialogs` coverage)
While implementing the theme-detection toggle above, a follow-up investigation into adding real unit tests for `DarkThemeSupport`'s widget-tree/palette-application logic (not just the new pure `isDarkModeActive()`/`WindowsSystemThemeDetector` pieces, which are already covered) confirmed this is currently **not safely achievable**:
- A throwaway experiment confirmed a real `new Display()` + `new Shell()` can be created successfully on an interactive Windows host, but `.github/workflows/maven.yml` runs CI on `runs-on: ubuntu-latest` with `mvn -B compile package` and **no Xvfb/X server configured**. Any test that constructs a real `Display` would pass locally but crash CI.
- There is **no existing precedent** in this suite for mocking concrete SWT widget-tree types (`Composite`/`Control`/`Menu`/`Color`, etc.) with Mockito — every prior headless technique in this project (see "Note on testing SWT event classes without a `Display`" above) either uses `SwtPlatformAssumptions.assumeNativePlatformMatches()` to skip on mismatched platforms, or Objenesis to bypass a `Widget`'s `Display`-bound constructor for otherwise-inert placeholder objects — neither technique extends to verifying actual recursive style/color application across a live control tree.
- **Decision (per explicit user instruction):** this item stays **documented as pending/blocked** rather than resolved with a workaround. It should be revisited only once the project adds a headless SWT test harness (Xvfb in CI, or equivalent) — tracked as Group 8 above and item 1 in "Future Testing Enhancements" below. Until then, `DarkThemeSupport`/`DarkJFaceDialogs`' widget-tree-facing logic (as opposed to the new pure `isDarkModeActive()` gating logic, already covered indirectly via `JISOCreatorThemeOptionsTest`/`WindowsSystemThemeDetectorTest`) remains without dedicated unit test coverage, consistent with the rest of Group 8.
...
LoadOSDirectoryContentsTaskTest.java:            4 tests  ← renamed
...
1. Complete the Test Gap Analysis plan above (Group 8): the full-GUI/SWT classes, which will also cover the 6 instance managers/singletons deferred from Group 6, `util/ImageUtils` deferred from Group 7, and `gui/theme/DarkThemeSupport`/`DarkJFaceDialogs` (see "Open item — headless SWT/`Display` test harness still pending" above) — all require a headless SWT harness (Xvfb) with a real `Display`. **Status: pending, explicitly left as an open decision by the user** until this infrastructure work is undertaken; do not attempt partial/mocked workarounds in the meantime.
...
7. **In progress on `feature/v0.2.3-windowsrootfiles` — Windows fixed-disk root visibility bugfix**: `ShowOnlyDirectoriesFilter` lacks the root short-circuit already present in `ShowOnlyIsoDirectoriesFilter` (`node.isRoot() ? true : isDirectory(node)`), so filesystem roots (`File.listRoots()`) whose `Files.isDirectory(Path)` check fails due to an I/O error (e.g. Windows Defender "Controlled folder access" blocking an unrecognized app from a fixed/internal drive, a locked BitLocker volume, restrictive NTFS ACLs) are silently hidden from the OS explorer tree, while removable media mounted afterward — usually not subject to the same restriction — shows up correctly. See "Known Issues" in `README.md` for the full root-cause analysis. Planned tests: distinguish an `AccessDeniedException` (root should still be shown, with a logged warning) from other I/O errors (e.g. not-ready/no-media removable drive, which should remain hidden, preserving existing coverage in `ShowOnlyDirectoriesFilterTest#shouldNotSelectFileRoots`), reproduced cross-platform via `Files.setPosixFilePermissions` on Linux with `Assumptions.assumeTrue` graceful skips where unsupported (same convention as the symlink regression tests). Being developed and exhaustively tested on the dedicated branch before merging into `feature/v0.2.3`.
