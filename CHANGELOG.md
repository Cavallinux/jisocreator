...
- **Busy-cursor feedback for background directory loading**: `LoadOSDirectoryContentsThread` now switches the `Shell` owning the OS directories table to the platform's `SWT.CURSOR_WAIT` busy cursor (a shared system cursor requiring no disposal) as soon as it is constructed, and restores the default cursor once the winning (still-latest) request finishes applying its result — giving the user immediate visual feedback while a large directory loads in the background. `run()` was also hardened with a `try`/`catch` around the attribute pre-fetch so the cursor is always restored even if the background scan fails.
- **Test updates for the above**: `LoadOSDirectoryContentsThreadTest` grew to **4 tests** (new `busyCursorMethodsShouldBeNoOpsWithoutTableViewer`, verifying the new cursor methods are safe no-ops without a real `TableViewer`). Suite grew to **279 tests / 63 classes**, verified passing under both `mvn -o clean test` (279/279, 0 skipped) and `mvn -o clean test -Pwindows` (279/279, 7 skipped, 0 failures).
- **Shared single-thread executor for background directory loading**: `LoadOSDirectoryContentsThread` (a `Thread` subclass, `.start()`'d per directory selection) was renamed to **`LoadOSDirectoryContentsTask`**, now a `Runnable` submitted to a shared single-thread `ExecutorService` instead. The new `submit()` method shows the busy cursor and cancels the previously submitted `Future` (via `cancel(false)`) before enqueueing the new task: since the executor is single-threaded, a still-queued (not yet started) previous task is removed entirely and never runs, so rapid directory navigation no longer accumulates wasted background scans. A task that has already started running when superseded is left to finish, its result still discarded via the existing `isStillLatestRequest()` check.
- **Test updates for the above**: `LoadOSDirectoryContentsThreadTest` was renamed to `LoadOSDirectoryContentsTaskTest` (still **4 tests**), updating only the `instanceof Thread` assertion to `instanceof Runnable`; all other pure-logic tests are unchanged. Suite size unchanged at **279 tests / 63 classes**, verified passing under both `mvn -o clean test` (279/279, 0 skipped) and `mvn -o clean test -Pwindows` (279/279, 7 skipped, 0 failures).
- **LRU multi-directory attributes cache**: `OSExplorer#warmAttributesCache(Path)` used to clear the entire attributes cache before repopulating it for the newly loaded directory, so only the single most recently loaded directory ever benefited from cached attributes — navigating back to a parent or a recently visited sibling directory always re-triggered a full filesystem scan. `OSExplorer#attributesCache` is now a `Map<Path, Map<Path, BasicFileAttributes>>` keyed by directory, backed by a synchronized, access-ordered `LinkedHashMap` that keeps up to **`MAX_CACHED_DIRECTORIES` (5)** directories' worth of entries at once, evicting only the least-recently-warmed directory once that limit is exceeded. Warming a directory now stores its entries under its own key instead of discarding everything else; `isDirectory`/`length`/`lastModifiedInstant` share a new private `cachedAttributes(Path)` lookup helper that resolves the target path's parent directory before checking the cache.
- **Test updates for the above**: `OSExplorerTest`'s `testWarmAttributesCacheClearsPreviousEntries` (asserted the old "discard everything else" behavior) was replaced with `testWarmAttributesCacheKeepsMultipleRecentDirectoriesCached` (warms 2 directories, then asserts a deleted file's *cached* size is still returned from the first one) and `testWarmAttributesCacheEvictsLeastRecentlyWarmedDirectoryBeyondCapacity` (warms 6 directories in sequence, one past the LRU capacity, then asserts the first one's entries were evicted). Suite grew to **282 tests / 63 classes**, verified passing under both `mvn -o clean test` (282/282, 0 skipped) and `mvn -o clean test -Pwindows` (282/282, 7 skipped, 0 failures — skip count unchanged, no new native/`Display`-dependent tests were introduced).

### Fixed
- **Symbolic-linked directories rendered as files in the OS explorer**: the attributes-cache optimization above (`OSExplorer#warmAttributesCache(Path)`) read each entry's attributes with `Files.readAttributes(entry, BasicFileAttributes.class, LinkOption.NOFOLLOW_LINKS)`. `NOFOLLOW_LINKS` reports a symbolic link itself as never being a directory, even when its target is one — so any directory reached via a symlink started showing up as a plain file (wrong icon, not expandable, filtered out of directory-only views) once its attributes came from the cache instead of a live filesystem check.
  - **`OSExplorer#warmAttributesCache(Path)`**: now calls `Files.readAttributes(entry, BasicFileAttributes.class)` (no `LinkOption`), following symbolic links — consistent with every uncached fallback in the class (`Files.isDirectory(Path)`, `Files.size(Path)`, `Files.getLastModifiedTime(Path)`), which already followed links by default.
  - **`OSExplorer#lastModifiedInstant(Path)`**: its uncached fallback was also changed from `Files.getLastModifiedTime(path, LinkOption.NOFOLLOW_LINKS)` to `Files.getLastModifiedTime(path)`, removing the one remaining `NOFOLLOW_LINKS` usage so cache-hit and cache-miss paths can never disagree on symlink handling. The now-unused `java.nio.file.LinkOption` import was removed.
- **Test updates for the above**: `OSExplorerTest` grew to **21 tests** (new `testWarmAttributesCacheFollowsSymbolicLinksToDirectories`, `testIsDirectoryFollowsSymbolicLinksToDirectoriesWithoutWarmedCache`), both using a shared `createSymbolicLinkOrAssumeUnsupported(...)` helper that gracefully skips (via `Assumptions.assumeTrue`) on filesystems/platforms without symlink-creation support instead of failing. Suite grew to **281 tests / 63 classes**, verified passing under both `mvn -o clean test` (281/281, 0 skipped) and `mvn -o clean test -Pwindows` (281/281, 7 skipped, 0 failures — skip count unchanged, the new tests are not native-SWT-dependent).
- **Windows: fixed-disk root drives missing from the OS explorer tree, while removable/USB drives show correctly** (implemented on the dedicated `feature/v0.2.3-windowsrootfiles` branch, pending exhaustive manual testing on real Windows 10/11 hardware before merging into `feature/v0.2.3`): root-cause analysis found that `ShowOnlyDirectoriesFilter` (the `TreeViewer` filter for the OS explorer) called `OSExplorer#isDirectory(Path)` for every element, including filesystem roots from `File.listRoots()`, without a root-aware special case — unlike its sibling `ShowOnlyIsoDirectoriesFilter`, which already short-circuits root nodes (`node.isRoot() ? true : isDirectory(node)`). Since `Files.isDirectory(Path)` (NIO.2) returns `false` both when a path genuinely isn't a directory and when any I/O error occurs reading its attributes, a fixed disk blocked by Windows Defender's "Controlled folder access" (or a locked BitLocker volume, restrictive NTFS ACLs, etc.) was silently filtered out of the tree, while removable media mounted afterward — typically not subject to the same OS-level restriction — showed up correctly. See the "Known Issues" section in `README.md` for the full analysis.
  - **`OSExplorer#isAccessibleRoot(Path)`** (new): reads a root path's `BasicFileAttributes` directly via `Files.readAttributes(...)` instead of the exception-swallowing `Files.isDirectory(Path)`. A successful read trusts `isDirectory()` as before; an `AccessDeniedException` is treated as "still a directory, just access-restricted" and returns `true` (logging a warning to help diagnose AV/Defender blocking); any other `IOException` (e.g. a not-ready removable/optical drive) returns `false`, preserving existing behavior for drives that truly aren't browsable.
  - **`ShowOnlyDirectoriesFilter#select()`**: now applies the same root short-circuit pattern already used by `ShowOnlyIsoDirectoriesFilter` — `isRoot(path) ? isAccessibleRoot(path) : isDirectory(path)` — so an access-restricted root is still shown, while non-root entries keep using the existing `isDirectory(Path)` check unchanged.
- **Test updates for the above**: `OSExplorerTest` grew to **25 tests** (new `testIsAccessibleRootReturnsTrueForReadableDirectory`, `testIsAccessibleRootReturnsFalseForNonExistentPath`, `testIsAccessibleRootReturnsTrueWhenAccessIsDenied` — the last reproduces a real `AccessDeniedException` on Linux by revoking execute/search permission on a temp directory's parent, gracefully skipping via `Assumptions.assumeTrue` where unreproducible, e.g. running as root); `ShowOnlyDirectoriesFilterTest` grew to **5 tests** (new `shouldSelectRootWhenAccessIsDenied`, an end-to-end regression test using the same technique plus a temporary `OSExplorer#setRoots(File[])` swap restored in a `finally` block). Suite grew to **286 tests / 63 classes**, verified passing under both `mvn -o clean test` (286/286, 0 skipped) and `mvn -o clean test -Pwindows` (286/286, 7 skipped, 0 failures — skip count unchanged, the new tests are not native-SWT-dependent).

### Planned
- **`ILazyContentProvider` migration for the OS explorer — deferred to `v0.3.0`**: the last remaining step of the OS file-system explorer performance work started in this branch. Would migrate `OsTableProvider`/the OS directories `TableViewer` to `ILazyContentProvider` (virtual scrolling via `setUseHashlookup(true)` + the `updateElement(int)` contract), loading only the currently visible rows on demand rather than the entire directory listing at once — the highest remaining impact for folders with thousands of entries. This is an architecture change with meaningfully higher risk than the increments already shipped in `feature/v0.2.3` (busy-cursor feedback, single-thread executor, symlink fix, LRU multi-directory attributes cache) — it can affect row ordering and selection-restoration behavior — so it is intentionally postponed until `v0.3.0`, with dedicated extensive testing, rather than merged into this branch.
